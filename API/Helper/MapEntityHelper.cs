...
namespace API.Helper
{
    public class MapEntityHelper
    {
        public static EntityByPage<T> MapEntityPaginationAsync<T>(string pageString, IEnumerable<T> list, float pageSize = 12f)
        {
            ...
            try
            {
                currentPage = CurrentPage(pageString, list.Count(), pageSize);
            }
            catch (Exception ex)
            {
            ...
            var resultPage = new EntityByPage<T>()
            {
                List = list.Skip(currentPage * (int)pageSize).Take((int)pageSize),
        ...
        private static int CurrentPage(string pageString, int TotalElements, float pageResults)
        {
            int page;
            int currentPage;
